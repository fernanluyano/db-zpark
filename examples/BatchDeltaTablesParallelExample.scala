...
/**
 * Example demonstrating parallel batch processing subtasks for multiple delta tables.
 * Tables are discovered dynamically.
 */
object BatchDeltaTablesParallelExample extends WorkflowTask {
  ...
  override protected def startTask: ZIO[TaskEnvironment, Throwable, Unit] =
    for {
      ...
      _        <- runSubtasksInParallel(subtasks)
      _        <- ZIO.logInfo("All discovered delta tables processed successfully in parallel")
    } yield ()
  ...
  private def runSubtasksInParallel(subtasks: Seq[DeltaTableSubtask]) =
    // Handle the case where no tables were found
    if (subtasks.isEmpty) {
      ZIO.logWarning("No tables found to process")
    } else {
      // Create and run the parallel runner with 4 concurrent tasks
      val runner = Factory.ParallelRunner(subtasks, 4)
      runner.run
    }
  ...
  class DeltaTableSubtask(
    ...
    override protected val ignoreAndLogFailures: Boolean = tableName == "table_1"
